...
region_df = region_df %>% mutate(cost_amt = (cost/amt)*1000)



p1 = ggplot(data = region_df, aes(x = policydate, y = cost_amt, group = state, color = state)) +
      theme_bw() + 
      geom_line() + 
      geom_point() + 
...
NE_df = NE_df %>%
      ...
      summarise_by_time(.date_var = policydate, .by= "day", 
                        cost = mean(policycost),
                        amt = mean(building_ins + content_ins)) %>%
      mutate(cost_amt = (cost/amt)*1000)



ts_st = ts(NE_df$cost_amt, start = as.yearmon(NE_df$policydate[1]), freq = 24*15)
...
ts_st = ts(MW_df$cost_amt, start = as.yearmon(MW_df$policydate[1]), freq = 24*15)
decom_st = decompose(ts_st)
...
ts_st = ts(S_df$cost_amt, start = as.yearmon(S_df$policydate[1]), freq = 24*15)
decom_st = decompose(ts_st)
...
ts_st = ts(MD_df$cost_amt, start = as.yearmon(MD_df$policydate[1]), freq = 24*15)
decom_st = decompose(ts_st)
...
ts_st = ts(PA_df$cost_amt, start = as.yearmon(PA_df$policydate[1]), freq = 24*15)
decom_st = decompose(ts_st)
